...
#define UPLOAD_FILE_RESUME "upload file resume"  // 断点续传中
...
/*
 * 文件传输协议扩展（断点续传）：
 * 上传请求 (ENUM_MSG_TYPE_UPLOAD_FILE_REQUEST):
 *   caData格式: "%s %lld %lld"  (文件名 文件总大小 已上传大小)
 *   caMsg: 服务器路径
 * 下载请求 (ENUM_MSG_TYPE_DOWNLOAD_FILE_REQUEST):
 *   caData格式: "%s %lld"  (文件名 已下载大小)
 *   caMsg: 服务器路径
